<?php
    ...
    include 'connect.php';
    $Tanggal=$_GET['tgl'];
    $select=mysqli_query($connect, "SELECT * FROM kas WHERE Tanggal='$Tanggal'");
        ...
        <div class="row text-center">
            ...
            <h2>Edit Data</h2>
                        <form action="edit.php" method="post" enctype="multipart/form-data">
                                ...
                                <input type="text" class="form-control" name="Tanggal" id="Tanggal" readonly value="<?php echo $show['Tanggal'];?>" required>
                                <label for="Kategori" class="form-label">Kategori</label>
                                <select class="form-select" name="Kategori" id="Kategori"  required aria-label="Default select example" > 
                                    <?php
                                    if ($show['Debet']>0)
                                    {
                                        ?><option readonly selected value="1">Debet</option>
                                          <option value="2">Kredit</option><?php
                                    }
                                    elseif($show['Kredit']>0)
                                    {
                                        ?><option value="1">Debet</option>
                                          <option readonly selected value="2">Kredit</option>
                                          <?php
                                    }
                                    ?>
                                </select>
                                ...
                                <label for="Jumlah" class="form-label">Nominal</label>
                                    ...
                                    ?>
                                <input value="<?php echo $Jumlah;?>" class="form-control" name="Jumlah" id="Jumlah" type="number" min="100" required>
                                <label for="Keterangan" class="form-label">Keterangan</label>
                                    <?php
                                    if ($show['Debet']>0){
                                        $Keterangan = $show['KetD'];
                                    }
                                    elseif($show['Kredit']>0){
                                        $Keterangan = $show['KetK'];
                                    }
                                    ?>
                                <input type="text" class="form-control" id="Keterangan" name="Keterangan" value="<?php echo $Keterangan; ?>"  required>
                                <label for="Bukti" class="form-label">Bukti</label>
                                <img src="proofs/<?php echo $show['Bukti']; ?>" alt="" style="width:3cm;height:3cm;">
